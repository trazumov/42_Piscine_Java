...
public class Transaction {
    ...
    public String toString() {
        String res = ANSI_GREEN + "Transaction{ ";

        if (this.category == TransferCategory.DEBIT) {
            res += this.recipient.getName() + " -> " + this.sender.getName() + ", +"
                    + this.amount + ", DEBIT, " + this.id + " }" + ANSI_RESET;
        } else {
            res += this.sender.getName() + " -> " + this.recipient.getName() + ", "
                    + this.amount + ", CREDIT, " + this.id + " }" + ANSI_RESET;
        }
